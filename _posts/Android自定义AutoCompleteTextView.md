# Android自定义文本输入提示和补全
=====================
  * 移动客户端上用户输入不方便，尽可能的帮助用户输入信息可以提高用户体验，这里要介绍的是Android中EditView的提示和补全控件----AutoCompleteTextView，为了满足应用的需求，这是介绍的是自定义控件的使用,废话不多说直接上代码：
...
public class EmailAutoCompleteTextView extends AutoCompleteTextView {
	...
	private void init(final Context context) {
		...
		this.setAdapter(new EmailAutoCompleteAdapter(context,
				R.layout.item_autocomplete, emailSufixs));
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					...
					if (text != null
						...
						// i.setBackgroundResource(R.drawable.images);
						// to.setView(i);
						// to.show();
					} else {
	...
	/*
	 * 自定义的提示listView采用自定义的布局，可以为所欲为了！！！
	 */
